...
- `src/sections/` — `Menu.tsx` (no menu list on the site, by the user's decision 2026-09-22: a pinned 320svh stage on the purple where the cups rise, trade, then part round "See the full menu", the link to Yousual; words step by `--m`; rises over the end of the dive with `margin-top: -55svh` so there's no blank stretch; phones: the cups' photo, the same words, the link), `Story.tsx` (left: sticky purple panel where the gold stag rises and the real lockup is uncovered HART → GROUND → full, by `--s`; right: four plain chapters scrolling past. The user rejected text laid over stag graphics), `Loyalty.tsx` (pinned 400svh on purple: big wallet card, stamps pressed by scroll `--p`, flips to the reward; links to yousual loyalty), `MapZoom.tsx` (canvas map zooming London → Richmond Park deer → the door; data `public/map/map.json` built by `node scripts/build-map.mjs` from OpenStreetMap, keep the © credit).
- Cups: `public/models/{coffee,matcha,hot}.glb` are Meshy models (unit height, base at y=0). Their baked logo was painted out of the textures; `CupPrint.tsx` prints the real logo (`public/cup-logo.svg`) and the back line on the wall (`kind="cold"` burgundy ink on the clear cups, `kind="hot"` gold foil on the paper cup), `cupFinish.ts` adds the finish (`finishCup`: ice/condensation/rim on the clear cups; `finishHot`: matte paper, glossy crema). Use both wherever the cups appear.
- The hot cup (menu section, in front of the matcha; `scene/Steam.tsx` puts a wisp of steam over it) was made 2026-09-22: Higgsfield image of the real paper cup with the lid off (`brand/gen/hot-b.png`) → Meshy image-to-3D (`blender/raw/hot-meshy.glb`) → `blender -b -P blender/prep_hot.py -- blender/raw/hot-meshy.glb public/models/hot.glb`. The script decimates, flattens the wall (Meshy raised the foil as relief), paints the print out by colour + a baked position mask, drops the normal/roughness maps (its atlas seams were baked into them as scratches) and exports Draco. `public/cups-still.webp` is a snap of the two cups off the live canvas (`?snap`).
